...
public class HtmlParser {
        ...
        try {
            ...
            for (Element child : childElements) {
                ...
                if (HtmlParserConfigEnum.PARSER_TYPE_ENTITY.getName().equals(qname)) {
                    ...
                    //获取list中要转换的class名称,全路径
                    String entityClassName = child.attributeValue("entityClassName");
                    ...
                    //写入缓存
                    map.put(TRANSFERLIST_CACHE_PREFIX + "_" + entityClassName + "_" + type, m);
                }
     ...
     *
     * @param htmlInfo   页面代码
     * @param clazz      需要转成的实体类
     * @param entityType 实体属性， 一般用于解析多个页面时使用， 可为空
     * @return
     */
    ...
    public static <T> T transferHtml(org.jsoup.nodes.Element htmlelem, Class<T> clazz, String entityType) {
        ...
        String mapKey = TRANSFER_CACHE_PREFIX + className;
        if (StringUtils.isNotBlank(entityType)) {
            mapKey = mapKey + "_" + entityType;
        }
     ...
     *
     * @param elems      待转换的doc文档
     * @param clazz      待转换到的实体对象,将根据clazz的全路径名称找需要转换的对象
     * @param listType   类型,对DTO时有效,对应list配置中的type字段.
     * @param entityType 要转换的实体，tranfer中的type
     * @param <T>
     * @return
     */
